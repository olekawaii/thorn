  ...
  Right (mods,target,args) ->
    ...
    concat . zipWith number args <$> mapM readFile args >>= (
      (
        ...
        pure . formatSh mods header date $ map (renderer . chunksOf (width header)) gif 
      ) 
      >>> \case
        Left e -> exitWithError e
...
parseArgs ("-f":fps:xs) m = case readMaybe fps :: Maybe Float of 
  Nothing -> Left $ ArgError "-f flag expected a \x1b[33mFLOAT\033[0m"
  Just x  -> parseArgs xs m {fps = 1.0 / x}
parseArgs ("-h":_)     _ = Left $ Help 
parseArgs (('-':x):_)  _ = Left . ArgError $ "Unknown argument " <> show (Colored Yellow ('-':x))
